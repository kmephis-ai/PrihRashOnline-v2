...
function resolveLocalModule(root, parentId, request) {
  ...
  const base = path.resolve(resolvedRoot, path.dirname(parentId), request);
  ...
  for (const candidate of candidates) {
    ...
    if (fs.existsSync(candidate) && fs.statSync(candidate).isFile()) {
      return normalizeId(path.relative(resolvedRoot, candidate));
    }
  }
...
function collectModules(root, entryModules = ENTRY_MODULES) {
  ...
  while (queue.length) {
    ...
    const fullPath = path.join(resolvedRoot, id);
    ...
    while ((match = REQUIRE_RE.exec(source))) {
      ...
      const resolved = resolveLocalModule(resolvedRoot, id, request);
...
function buildRuntimeBundleSource(sourceRoot, entryModules = ENTRY_MODULES) {
  ...
  const modules = collectModules(root, entryModules);
  const lines = [
  ...
  lines.push(
    ...
    '}',
    'var runtime={schema:__schema,version:"1.0.0",generated_from_canonical_lib:true,financial_formula_copy:false};'
  );
  Object.entries(entryModules).forEach(([name, id]) => {
