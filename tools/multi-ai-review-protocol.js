...
function assertPublicSafe(value) {
  ...
  if (forbidden.some((pattern) => pattern.test(serialized))) fail('MULTI_AI_PRIVATE_CONTEXT_FORBIDDEN');
  return true;
}
...
function validatePacket(packet) {
  ...
  if (!Array.isArray(packet.changed_paths) || packet.changed_paths.length === 0
      || packet.changed_paths.length > 500 || new Set(packet.changed_paths).size !== packet.changed_paths.length) {
    fail('MULTI_AI_CHANGED_PATHS_INVALID');
  }
  if (!Array.isArray(packet.public_evidence_refs) || packet.public_evidence_refs.length > 200
      || new Set(packet.public_evidence_refs).size !== packet.public_evidence_refs.length) {
    fail('MULTI_AI_EVIDENCE_REFS_INVALID');
  }
  for (const value of [...packet.changed_paths, ...packet.public_evidence_refs]) {
    if (typeof value !== 'string' || value.length < 1 || value.length > 240) fail('MULTI_AI_REFERENCE_INVALID');
  }
...
function normalizeFinding(finding) {
  const allowed = ['severity', 'code', 'path', 'summary', 'recommendation', 'resolved'];
...
function validateReport(report, packet) {
  ...
  if (report.roadmap_id !== packet.roadmap_id || report.issue !== packet.issue || report.pr !== packet.pr
      || report.candidate_sha !== packet.candidate_sha) {
    fail('MULTI_AI_CANDIDATE_IDENTITY_MISMATCH');
  }
...
function aggregateReviewReports(packetInput, reportsInput) {
  try {
    ...
    for (const report of reports) {
      if (roleSet.has(report.role)) fail('MULTI_AI_DUPLICATE_ROLE');
      if (reviewerSet.has(report.reviewer_id)) fail('MULTI_AI_DUPLICATE_REVIEWER');
      roleSet.add(report.role);
      reviewerSet.add(report.reviewer_id);
    }
    const missingRoles = REQUIRED_ROLES.filter((role) => !roleSet.has(role));
    if (missingRoles.length) {
      return {
        status: 'INCOMPLETE',
        reasonCode: 'MULTI_AI_REQUIRED_ROLE_MISSING',
        candidateSha: packetInput.candidate_sha,
        rolesPresent: Array.from(roleSet).sort(),
        missingRoles,
        blockingFindings: 0,
        advisoryFindings: 0,
        reviewerCanMarkDone: false,
        reviewerHasWriterAuthority: false
      };
    }

    const allFindings = reports.flatMap((report) => report.findings.map((finding) => ({ ...finding, role: report.role })));
    ...
    return {
      ...
      candidateSha: packetInput.candidate_sha,
      rolesPresent: REQUIRED_ROLES.slice(),
      missingRoles: [],
      blockingFindings: blocking.length,
      advisoryFindings: advisory.length,
      reviewerCanMarkDone: false,
      reviewerHasWriterAuthority: false,
...
function evaluateSupplementaryEvidence(reviewResult, machineEvidence) {
  ...
  const requiredMachine = ['prValidation', 'trustedDevDeploy', 'trustedRuntimeHealth', 'autonomousMerge', 'mainVerification'];
  const machinePass = requiredMachine.every((key) => String(evidence[key] || '') === 'PASS');
  return {
