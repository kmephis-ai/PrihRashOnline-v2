/**
 * UI-REC-001 / DATA-REC-001 / FIN-REC-001 truthful canonical R2 Web App router.
 */
var PRH_CANONICAL_R2_WEB = Object.freeze({
  ...
  SAFE_UNBOUND_SURFACES: Object.freeze({ budget: 'Бюджет', obligations: 'Обязательства' }),
...
function prhR2RouteHref_(surface, extraParams) {
  ...
  return (prhR2SelfUrl_() || '') + '?' + parts.join('&');
}
...
function prhR2PrivacyRouteScript_() {
  return '<script id="prh-r2-privacy-route-script">(function(){' +
    ...
    'function apply(){document.querySelectorAll("#prh-r2-shell a,#dq-link,#tx-link,a.back,a.drill").forEach(preserve);}' +
...
function prhR2NavigationHtml_(activeSurface, routeParams) {
  ...
  var primary = PRH_CANONICAL_R2_WEB.NAVIGATION.map(function(item) {
    var current = item[0] === activeSurface ? ' aria-current="page"' : '';
    return '<a data-r2-nav="' + prhR2EscapeHtml_(item[0]) + '"' + current + ' href="' + prhR2EscapeHtml_(prhR2RouteHref_(item[0], privacyParams)) + '">' + prhR2EscapeHtml_(item[1]) + '</a>';
  }).join('');
  ...
  var css = '#prh-r2-shell{position:sticky;top:0;z-index:1000;display:flex;align-items:center;gap:10px;max-width:100%;min-width:0;padding:8px 12px;background:#061d37;border-bottom:1px solid rgba(255,255,255,.16);font:600 13px/1.2 Inter,system-ui,sans-serif}' +
    '#prh-r2-canonical-nav,#prh-r2-secondary-nav{display:flex;align-items:center;gap:6px;min-width:0}' +
    '#prh-r2-secondary-nav{margin-left:auto}#prh-r2-shell a{color:#dbeafe;text-decoration:none;padding:8px 10px;border-radius:999px;border:1px solid transparent;white-space:nowrap}' +
    '#prh-r2-shell a:hover,#prh-r2-shell a:focus-visible{background:#123f66;color:#fff}#prh-r2-shell a[aria-current="page"]{background:#fff;color:#061d37}' +
    '#prh-r2-secondary-nav a{border-color:rgba(255,255,255,.18);color:#bfdbfe}' +
    '@media(max-width:1100px){#prh-r2-shell{align-items:flex-start;flex-direction:column}#prh-r2-canonical-nav,#prh-r2-secondary-nav{width:100%;max-width:100%;overflow-x:auto;overscroll-behavior-x:contain}#prh-r2-secondary-nav{margin-left:0}}';
  return '<div id="prh-r2-shell" data-prh-canonical-r2-shell="1" data-navigation-policy="PROVEN_DESTINATIONS_ONLY" data-route-link-mode="SELF_URL" data-privacy-route-policy="PRESERVE_EXPLICIT_MODE" data-active-surface="' + prhR2EscapeHtml_(activeSurface) + '">' +
    ...
    '<nav id="prh-r2-secondary-nav" aria-label="Дополнительные инструменты PrihRashOnline">' + secondary + '</nav></div><style id="prh-r2-canonical-nav-style">' + css + '</style>' + prhR2PrivacyRouteScript_();
}
function prhR2InjectShell_(html, activeSurface, routeParams) {
  ...
  if (selfUrl) {
    ...
    if (html.indexOf('<base target="_top">') >= 0) html = html.replace('<base target="_top">', baseTag);
    else if (html.indexOf('<head>') >= 0 && html.indexOf('<base ') < 0) html = html.replace('<head>', '<head>' + baseTag);
  }
...
function prhR2HardenPrivateHome_(html) {
  ...
  if (html.indexOf('R2_PRIVATE_HOME_PAYLOAD_REQUIRED') < 0 || html.indexOf(previewParser) >= 0) throw new Error('R2_HOME_PRIVATE_FAIL_CLOSED_HARDENING_FAILED');
  return html;
}
function prhR2RenderFile_(surface, payload, routeParams) {
  ...
  var html = HtmlService.createHtmlOutputFromFile(spec.file).getContent();
  if (spec.placeholder) {
  ...
  } else if (payload != null) throw new Error('R2_STATIC_SURFACE_PAYLOAD_FORBIDDEN');
  if (surface === 'home') html = prhR2HardenPrivateHome_(html);
  html = prhR2InjectShell_(html, surface, routeParams);
  return HtmlService.createHtmlOutput(html).setTitle('PrihRashOnline — ' + spec.title).addMetaTag('viewport', 'width=device-width, initial-scale=1');
}
function prhR2RenderUnavailable_(surface, routeParams) {
  ...
  var privacyParams = prhR2RoutePrivacyParams_(routeParams);
  var html = '<!doctype html><html lang="ru"><head><base target="_top"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>*{box-sizing:border-box}body{margin:0;background:#f3f6fa;color:#13243a;font:14px/1.5 Inter,system-ui,sans-serif}.r2-state{max-width:760px;margin:56px auto;padding:0 18px}.r2-card{background:#fff;border:1px solid #dbe3ec;border-radius:18px;padding:26px}.note{color:#617086}.back{display:inline-block;margin-top:18px;padding:9px 13px;border-radius:999px;background:#0b5d65;color:#fff;text-decoration:none;font-weight:700}</style></head><body><main class="r2-state"><section class="r2-card" data-r2-unbound-surface="' + prhR2EscapeHtml_(surface) + '"><div class="note">PrihRashOnline</div><h1>' + prhR2EscapeHtml_(title) + '</h1><p>Этот раздел ещё подключается к реальным данным. Мы не показываем здесь демонстрационные значения вместо ваших финансов.</p><a class="back" href="' + prhR2EscapeHtml_(prhR2RouteHref_('home', privacyParams)) + '">Вернуться на главную</a></section></main></body></html>';
  return HtmlService.createHtmlOutput(prhR2InjectShell_(html, surface, routeParams)).setTitle('PrihRashOnline — ' + title).addMetaTag('viewport', 'width=device-width, initial-scale=1');
}
function prhR2RenderLegacy_(params) {
  return prhRenderWebDashboard_(prhGetWebDashboardData(params.year, params.month, params.view));
}
function prhR2AsyncHomeMarker_(mode) {
  return { schema: 'PRH_R2_HOME_ASYNC_BOOTSTRAP_V1', version: '1.0.0', runtime_load: 'ASYNC', privacy_mode: mode };
}
function prhR2FetchFinancialHomePayload(privacyMode) {
  ...
  var transformed = prhPrivacyTransform_(prhR2BuildFinancialHomeRuntime_(), mode, PRH_PRIVACY_PRESENTATION_RUNTIME.PRIVATE_SOURCE);
...
function prhR2RenderHomeWithPrivacy_(params) {
  ...
  if (mode === 'ZEN') {
    var zen = prhPrivacyTransform_(prhR2BuildFinancialHomeRuntime_(), mode, PRH_PRIVACY_PRESENTATION_RUNTIME.PRIVATE_SOURCE);
    return prhPrivacyRenderZenCanonical_(zen);
  }
  return prhPrivacyDecorateOutput_(prhR2RenderFile_('home', prhR2AsyncHomeMarker_(mode), params), mode, PRH_PRIVACY_PRESENTATION_RUNTIME.PRIVATE_SOURCE, 'PrihRashOnline — Финансовый обзор');
}
function doGet(e) {
  ...
  if (['transactions', 'expenses', 'income', 'cash-flow', 'data-quality'].indexOf(surface) >= 0) return prhR2RenderFile_(surface, null, params);
  if (surface === PRH_CANONICAL_R2_WEB.STUDIO_SURFACE) return prhDashboardComposerDecorateStudioOutput_(prhPrivacyDecorateStudioOutput_(prhR2RenderFile_('studio', null, params), params[PRH_PRIVACY_PRESENTATION_RUNTIME.URL_PARAMETER]));
...
function prhR2SmokePayload_() {
  ...
  return {
    smoke: true, schema: 'PRH_FINANCIAL_HOME_VIEW_V1', contract_version: '1.0.0', currency: 'RUB',
    ...
    cards: { INCOME: unavailable('INCOME'), EXPENSE: unavailable('EXPENSE'), CASH_FLOW: unavailable('CASH_FLOW'), SAVINGS: unavailable('SAVINGS'), BUDGET: { id: 'BUDGET', state: 'NOT_CONFIGURED', budget_minor: null, expense_minor: null, variance_minor: null, currency: 'RUB', drill: null }, LIQUIDITY: { id: 'LIQUIDITY', state: 'UNAVAILABLE_PENDING_BALANCE_SOURCE', value_minor: null, currency: 'RUB', drill: null }, ALERTS: { id: 'ALERTS', state: 'READY', count: 0, highest_severity: null, currency: 'RUB', drill: null } },
    alerts: [], visual_data: { cash_flow_minor: [], expense_mix: [] }, provenance: { financial_values: 'TECHNICAL_SMOKE_ONLY', ui_financial_formula_used: false }
  };
}
function prhCanonicalR2WebAppSmokeToken() {
  var html = prhR2RenderFile_(PRH_CANONICAL_R2_WEB.DEFAULT_SURFACE, prhR2SmokePayload_()).getContent();
  if (!html || html.indexOf('data-prh-canonical-r2-shell="1"') < 0 || html.indexOf('data-navigation-policy="PROVEN_DESTINATIONS_ONLY"') < 0 || html.indexOf('data-route-link-mode="SELF_URL"') < 0 || html.indexOf('data-privacy-route-policy="PRESERVE_EXPLICIT_MODE"') < 0 || html.indexOf('data-active-surface="home"') < 0 || html.indexOf('Финансовый обзор') < 0 || html.indexOf('"smoke":true') < 0 || html.indexOf('?surface=legacy') < 0 || html.indexOf('?surface=transactions') < 0 || html.indexOf('?surface=expenses') < 0 || html.indexOf('?surface=income') < 0 || html.indexOf('?surface=cash-flow') < 0 || html.indexOf('?surface=data-quality') < 0 || html.indexOf('Студия аналитики') < 0 || html.indexOf('R2_PRIVATE_HOME_PAYLOAD_REQUIRED') < 0) throw new Error('R2_CANONICAL_RENDER_SMOKE_FAILED');
  ['budget', 'obligations'].forEach(function(surface) { if (html.indexOf('?surface=' + surface) >= 0) throw new Error('R2_CANONICAL_FALSE_AFFORDANCE_PRESENT'); });
