...
assert.strictEqual(preserveAll.status, 'BLOCKED');
assert(preserveAll.blockers.includes('CANONICAL_IDENTITY_EXTENSION_REQUIRED'));
assert.strictEqual(preserveAll.canonical_candidate.length, 0);
...
console.log('mig010_repair_policy_contract_test: OK', {
  ...
  canonicalIdentityExtensionRequiredWhenNeeded: true,
