...
(async () => {
  ...
  try {
    for (const viewport of [{name:'desktop',width:1280,height:900},{name:'mobile',width:390,height:844}]) {
      ...
      assert.strictEqual(await page.locator('#catalog-status-dot').getAttribute('data-state'),'AVAILABLE',`${viewport.name} truthful verified catalog state`);
      assert.strictEqual((await page.locator('#catalog-status-text').textContent()).trim(),'Конфигурация проверена',`${viewport.name} truthful verified catalog copy`);
