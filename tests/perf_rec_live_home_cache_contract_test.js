...
function createHarness() {
  ...
  function privateHome() {
    ...
    context.prhPerfRecRecordSource_({
      ...
      range_read_count: 4,
      cell_read_count: 1500,
      canonical_snapshot_read_count: 1,
      snapshot_reuse_count: 0,
      unique_dimension_hash_count: 11,
      dimension_hash_memo_hit_count: 389,
      canonical_revision_hash_prefix: 'abcdef123456'
    });
...
assert.strictEqual(cold.canonical_revision_hash_prefix, 'abcdef123456');
assert(h.cache.size > 0, 'cold exact-revision build must seed private user cache');

const warm = h.context.prhPerfRecBaselineProbe('WARM');
assert.strictEqual(h.builderCalls, 1, 'warm exact-revision hit must not rebuild financial Home');
assert.strictEqual(warm.mode, 'WARM');
...
// Any source modified-time change produces a new PERF-011-derived cache identity.
h.setModified(2000);
...
assert.strictEqual(changed.reason_code, 'COLD_SINGLE_SCAN_BUILT');
assert.notStrictEqual(changed.source_revision_hash_prefix, warm.source_revision_hash_prefix);

// Source geometry is also bound into the cheap revision token.
...
// Concurrent source change during a cold materialization cannot seed or return a stale snapshot.
h.setModified(3000);
h.setMutateDuringBuild(true);
assert.throws(
  () => h.context.prhPerfRecBaselineProbe('COLD'),
  /PERF_REC_SOURCE_REVISION_CHANGED_DURING_BUILD/
);
h.setMutateDuringBuild(false);

// Public/trusted baseline telemetry contains no financial payload, labels, IDs, URL or cached Home object.
...
assert.match(source, /source_revision_hash_prefix/);
assert.doesNotMatch(source, /setValue\s*\(|setValues\s*\(|appendRow\s*\(/);

console.log('perf_rec_live_home_cache_contract_test: OK', {
  contract: 'PRH_PERF_REC_RUNTIME_V1@1.0.0',
  perf011CacheIdentity: true,
  exactSourceRevisionHit: true,
  revisionChangeMiss: true,
  concurrentChangeFailClosed: true,
  userPrivateCache: true,
  baselineFinancialPayload: false,
