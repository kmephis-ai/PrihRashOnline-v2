...
function createContext(options = {}) {
  ...
  const readCounter={value:0}, webSmokeCounter={value:0}, homeReadSmokeCounter={value:0}, dataSmokeCounter={value:0};
  ...
  const context={Date,Math,Number,String,Error,
  ...
  return{context,readCounter,webSmokeCounter,homeReadSmokeCounter,dataSmokeCounter};
}
...
assert.strictEqual(result.buildInfoSchemaVersion,1);assert.strictEqual(result.runtime,'V8');assert.strictEqual(result.requiredSheetCount,3);assert.strictEqual(result.readCheck,true);assert.strictEqual(result.dataRuntimeCheck,true);
assert(Number.isInteger(result.latencyMs)&&result.latencyMs>=0);
assert.strictEqual(healthy.readCounter.value,1);assert.strictEqual(healthy.webSmokeCounter.value,1);assert.strictEqual(healthy.homeReadSmokeCounter.value,1);assert.strictEqual(healthy.dataSmokeCounter.value,1);
...
console.log('runtime_health_contract_test: OK',{
  ...
  dataRuntimeSmoke:'V1_READ_ONLY',scalarShapePreserved:true,financialPayload:false
});
