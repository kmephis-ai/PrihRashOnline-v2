...
function createContext(options = {}) {
  ...
  const context={Date,Math,Number,String,Error,RegExp,Object,Array,
  ...
  if(!options.localFirstMissing)context.prhLocalFirstSyncBootstrap=function(request){
    ...
    if(options.localFirstInvalid)return options.localFirstInvalid;
    return {
      ...
      expected_counts:{transactions:0,dimensions:0,aggregates:0,sync_journal:0},serialized_chars:128
    };
  };
...
assert.strictEqual(tokenHealthy.localFirstCounter.value,1,'Local-first owner bootstrap proof must execute even though scalar shape is unchanged');
...
assert.throws(()=>createContext({localFirstMissing:true}).context.prhReleaseHealthCheck({candidateSha,sourceTreeHash}),/RUNTIME_HEALTH_LOCAL_FIRST_SYNC_MISSING/);
...
assert.throws(()=>createContext({localFirstInvalid:{schema:'bad',state:'FULL_BOOTSTRAP',financial_write_authorized:false,canonical_mutation_performed:false,serialized_chars:1}}).context.prhReleaseHealthCheck({candidateSha,sourceTreeHash}),/RUNTIME_HEALTH_LOCAL_FIRST_BOOTSTRAP_INVALID/);
...
console.log('runtime_health_contract_test: OK',{
  ...
  dataRuntimeSmoke:'V1_READ_ONLY',localFirstOwnerBootstrap:true,privacySafeOwnerBootstrapFailure:true,scalarShapePreserved:true,financialPayload:false
});
