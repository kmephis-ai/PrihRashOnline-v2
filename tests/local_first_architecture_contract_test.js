...
// Local-first is a read/runtime optimization, never a new financial authority.
...
// IndexedDB model is exact-revision, atomic-generation and rebuilds rather than guessing.
...
assert.deepStrictEqual(
  contract.local_read_model.stores,
  ['meta', 'transactions', 'dimensions', 'aggregates', 'sync_journal']
);

// Core warm UX property: no server/Sheets dependency in ordinary interaction path.
...
// Heavy analytics must not block the main UI thread and stale results cannot commit.
...
// Sync is background, exact-bound and fail-closed to rebuild when delta chain is uncertain.
...
// Target SLOs are explicit and deliberately separated from server health/cold bootstrap timings.
...
// Privacy and future YDB migration remain fail-closed.
...
// Human-facing architecture/roadmap must remain aligned with the machine contract.
for (const required of [
  ...
  'Big-bang cutover запрещён'
]) {
  assert(
    architecture.includes(required) || roadmap.includes(required),
...
console.log('Local-first architecture contract: PASS');
