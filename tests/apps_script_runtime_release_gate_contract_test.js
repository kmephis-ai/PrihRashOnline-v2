...
const html = fs.readFileSync('DashboardWebApp.html', 'utf8');
...
const templateOpeners = html.match(/<\?/g) || [];
if (templateOpeners.length !== 1 || !html.includes('<?!= initialData ?>')) {
  throw new Error('Prepared DashboardWebApp.html must contain exactly one real Apps Script template opener');
}
if (html.includes("text.indexOf('<?')")) {
  throw new Error('Client-side literal <? must never survive prepare:web; it is parsed as an Apps Script template opener');
}
if (!html.includes("text.indexOf('<' + '?')")) {
  throw new Error('Prepared DashboardWebApp.html must use the split template-sentinel check');
}
