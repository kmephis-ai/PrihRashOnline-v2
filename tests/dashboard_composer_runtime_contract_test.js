...
for (const route of ['transactions','expenses','income','cash-flow','data-quality']) {
  assert.strictEqual(context.PRH_CANONICAL_R2_WEB.ROUTE_TRUTH[route].navigation, 'PRIMARY');
  assert.strictEqual(context.PRH_CANONICAL_R2_WEB.ROUTE_TRUTH[route].binding_state, 'BOUND_READ_ONLY');
}
...
for (const route of ['home','transactions','expenses','income','cash-flow','data-quality']) {
  assert(composer.includes(`?surface=${route}`), `composer shell missing proven route ${route}`);
}
assert(!composer.includes('?surface=budget'));
assert(!composer.includes('?surface=obligations'));
...
for (const route of ['transactions','expenses','income','cash-flow','data-quality']) {
  assert(defaultHome.includes(`?surface=${route}`), `Home shell missing proven route ${route}`);
}
assert.strictEqual(legacyReads, 0);
...
console.log('dashboard-composer-runtime: PASS', {
  ...
  primaryNavigation: ['home','transactions','expenses','income','cash-flow','data-quality'],
