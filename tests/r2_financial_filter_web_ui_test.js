...
(async () => {
  ...
  try {
    ...
    await page.addInitScript(() => {
      window.__fixture = (section) => {
        const common = {
          ...
          privacy_mode:'NORMAL',currency:'RUB',window_days:90,snapshot_revision:'a'.repeat(64),snapshot_revision_prefix:'aaaaaaaaaaaa',
      ...
      const runner={success:null,failure:null,withSuccessHandler(fn){this.success=fn;return this;},withFailureHandler(fn){this.failure=fn;return this;},prhR2FetchFinancialSectionsPayload(request){const callback=this.success;const section=request.section;setTimeout(()=>callback(window.__fixture(section)),0);return this;}};
    ...
    const state=await page.evaluate(()=>{
      ...
      return {defaultPrevented:event.defaultPrevented,action:form.action,target:form.target,canonicalAction:form.dataset.canonicalAction,surface:data.get('surface'),privacy:data.get('privacy'),windowDays:data.get('window_days'),account:data.get('account_id'),category:data.get('category_id'),member:data.get('member_id')};
    });
    ...
    assert.deepStrictEqual({surface:state.surface,privacy:state.privacy,windowDays:state.windowDays,account:state.account,category:state.category,member:state.member},{surface:'expenses',privacy:'NORMAL',windowDays:'30',account:'ACC-1',category:'CAT-1',member:'MEM-1'});
