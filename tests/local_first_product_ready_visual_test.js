...
(async () => {
  ...
  async function fullJourney(device) {
    ...
    try {
      ...
      assert(/последняя проверенная локальная ревизия/i.test(degraded.banner));
  ...
  async function noActiveSnapshotFailsClosed() {
    ...
    try {
      ...
      assert(/verified snapshot|проверенная локальная ревизия/i.test(failed.text) || failed.text.trim()==='', 'fail-closed UI must not claim successful finance data');
