...
function prhBuildApplicationMenu() {
  ...
  const actionsMenu = ui.createMenu('Действия')
    .addItem('Обновить расчёты','prhRefreshIncomeDashboard')
...
function prhMenuCreatePdf() {
  if (typeof prhCreateIncomeDashboardPdf !== 'function') {
    return prhFeaturePending_('Экспорт PDF','Модуль экспорта будет подключён позже.');
  }
...
function prhMenuCreateSnapshot() {
  if (typeof prhCreateIncomeDashboardSnapshot !== 'function') {
    return prhFeaturePending_('Снимок показателей','Модуль снимков будет подключён позже.');
  }
...
function prhValidateDashboardApplication() {
  ...
  results.webDashboard = {
    status:'AVAILABLE',
    version:PRH_WEB_DASHBOARD.VERSION,
