...
const IDENTITY_STRATEGIES = Object.freeze([
  'EXTERNAL_ID',
...
function migrationCanonicalBase(input) {
  ...
  return { migration, fingerprint, sourceContainer, sourcePosition };
}

function buildMigrationCanonicalTransaction(input, identity) {
  const { migration, fingerprint, sourceContainer, sourcePosition } = migrationCanonicalBase(input);
...
function fromMigrationCanonicalRecord(input) {
  const { migration, fingerprint } = migrationCanonicalBase(input);
  return buildMigrationCanonicalTransaction(input, {
...
function fromMigrationCanonicalOccurrenceRecord(input, occurrenceOrdinal) {
  const ordinal = Number(occurrenceOrdinal);
  if (!Number.isInteger(ordinal) || ordinal < 1 || ordinal > 999999) {
    fail('CANONICAL_OCCURRENCE_ORDINAL_INVALID');
  }
  const { fingerprint } = migrationCanonicalBase(input);
  const ordinalText = String(ordinal).padStart(6, '0');
  return buildMigrationCanonicalTransaction(input, {
    transaction_id: `migocc-${fingerprint.slice(0, 40)}-${ordinalText}`,
    source_record_id: `sha256:${fingerprint}:occurrence:${ordinalText}`,
    identity_strategy: 'CONTENT_FINGERPRINT_OCCURRENCE_V1'
  });
}
...
function assertMigrationFingerprintParity(input) {
  const tx = input && input.schema === SCHEMA_ID ? input : normalizeCanonicalTransaction(input);
  if (!['CONTENT_FINGERPRINT_V1', 'CONTENT_FINGERPRINT_OCCURRENCE_V1'].includes(tx.provenance.identity_strategy)) {
    fail('CANONICAL_MIGRATION_IDENTITY_STRATEGY_REQUIRED');
  }
