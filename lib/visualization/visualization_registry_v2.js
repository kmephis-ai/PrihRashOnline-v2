...
function remapEncoding(sourceSpec, targetType, rule) {
  ...
  const target = CONTRACT.chart_registry[targetType];
  if (!target) fail('VIZ070_CHART_TYPE_UNKNOWN');
  const required = new Set(target.required_roles || []);
  const encoding = {};
  for (const [targetRole, sourceRole] of Object.entries(rule.mapping)) {
    const binding = sourceSpec.encoding[sourceRole];
    if (!binding) {
      if (required.has(targetRole)) fail('VIZ070_RETYPE_REQUIRED_BINDING_MISSING');
      continue;
    }
    encoding[targetRole] = { ...binding };
  }
  return encoding;
}
