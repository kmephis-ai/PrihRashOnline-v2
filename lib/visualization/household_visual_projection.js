...
function compileHouseholdChart(chartSpec, dataset) {
  const compiled = viz.compileEChartsOption(chartSpec, dataset);
  const description = String(compiled.option && compiled.option.title && compiled.option.title.text || 'Финансовый график');
  return Object.freeze({
    renderer: compiled.renderer,
    option: Object.freeze({
      ...compiled.option,
      aria: Object.freeze({
        ...(compiled.option.aria || {}),
        label: Object.freeze({ description })
      })
    })
  });
}
