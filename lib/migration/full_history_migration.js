...
function preflightExistingCanonical(sourceInputs, canonicalMigrationRecords) {
  const result = reconcileMigrations(sourceInputs, canonicalMigrationRecords);
  const reasons = new Set();
  if (result.summary.invalid_source_count > 0) reasons.add(REASON.SOURCE_INVALID);
  result.results.forEach((item) => {
    if (item.status !== 'CLEAN') reasons.add(item.reason || REASON.CORE_MISMATCH);
  });
  return Object.freeze({
...
function buildMigrationPlan(input) {
  ...
  const existingPreflight = preflightExistingCanonical(sources, targetMigration);
  const dryRun = planIdempotentImport(sources, targetMigration);
  const dryRunBlocked = dryRun.filter((item) => item.action === 'BLOCK');
  const blockedReasons = new Set(existingPreflight.reasons);
  dryRunBlocked.forEach((item) => blockedReasons.add(item.reason));
  ...
  const sortedBlockedReasons = Array.from(blockedReasons).sort();
  const identity = {
    ...
    existing_target_preflight: existingPreflight.status,
    blocked_reasons: sortedBlockedReasons,
  ...
  return Object.freeze({
    ...
    blocked_reasons: Object.freeze(sortedBlockedReasons),
