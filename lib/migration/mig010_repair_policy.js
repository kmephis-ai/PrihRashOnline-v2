...
const COMPATIBLE_PROPOSAL_POLICY_VERSIONS = Object.freeze(['1.0.0', '1.1.0']);
...
function uniqueCandidateIdentities(canonicalRecords) {
  const candidate = validateCanonicalCollection(Array.isArray(canonicalRecords) ? canonicalRecords : []);
  return candidate.map((tx) => ({
    transaction_id: tx.transaction_id,
    source_record_id: tx.provenance.source_record_id,
    source_fingerprint: tx.provenance.source_fingerprint
  }));
...
function proposalIdentity(fields) {
  return {
    policy_schema: fields.policy_schema,
    ...
    backup_cipher_sha256: fields.backup_cipher_sha256,
    mapping_version: fields.mapping_version,
    target_scope: fields.target_scope,
    invalid_quarantine: fields.invalid_quarantine,
    duplicate_groups: fields.duplicate_groups,
    unique_candidate_identities: fields.unique_candidate_identities
  };
}

function storedProposalHash(proposal) {
  if (!Array.isArray(proposal.target_scope) || !Array.isArray(proposal.invalid_quarantine) ||
      !Array.isArray(proposal.duplicate_groups) || !Array.isArray(proposal.unique_candidate)) {
    fail('MIG010_REPAIR_PROPOSAL_SHAPE_INVALID');
  }
  const identity = proposalIdentity({
    policy_schema: proposal.policy_schema,
    policy_version: proposal.policy_version,
    strategy: proposal.strategy,
    plan_hash: proposal.plan_hash,
    source_revision: proposal.source_revision,
    target_revision: proposal.target_revision,
    backup_cipher_sha256: proposal.backup_cipher_sha256,
    mapping_version: proposal.mapping_version,
    target_scope: proposal.target_scope,
    invalid_quarantine: proposal.invalid_quarantine,
    duplicate_groups: proposal.duplicate_groups,
    unique_candidate_identities: uniqueCandidateIdentities(proposal.unique_candidate)
  });
  return sha256(identity);
}

function assertProposalPolicyCompatibility(proposal) {
  if (!proposal || proposal.schema !== PROPOSAL_SCHEMA ||
      proposal.policy_schema !== 'MIG010_REPAIR_POLICY_V1' ||
      !COMPATIBLE_PROPOSAL_POLICY_VERSIONS.includes(text(proposal.policy_version)) ||
      proposal.strategy !== 'REBUILD_LEGACY_SLICE_V1' ||
      !SHA256_RE.test(String(proposal.proposal_hash || '')) ||
      !SHA256_RE.test(String(proposal.plan_hash || '')) ||
      !SHA256_RE.test(String(proposal.source_revision || '')) ||
      !SHA256_RE.test(String(proposal.target_revision || '')) ||
      !SHA256_RE.test(String(proposal.backup_cipher_sha256 || '')) ||
      !text(proposal.mapping_version) || text(proposal.mapping_version).length > 80) {
    fail('MIG010_REPAIR_PROPOSAL_POLICY_INCOMPATIBLE');
  }
  if (storedProposalHash(proposal) !== proposal.proposal_hash) {
    fail('MIG010_REPAIR_PROPOSAL_HASH_MISMATCH');
  }
  return true;
}
...
function buildRepairProposal(input) {
  ...
  const identity = proposalIdentity({
    ...
    unique_candidate_identities: uniqueCandidateIdentities(uniqueCandidate)
  });
...
function applyRepairResolution(input) {
  ...
  assertProposalPolicyCompatibility(proposal);
  const sourceRecords = Array.isArray(input.source_records) ? input.source_records : fail('MIG010_REPAIR_SOURCE_COLLECTION_INVALID');
