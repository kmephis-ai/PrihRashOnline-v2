...
function assertPolicy() {
  if (!POLICY || POLICY.schema !== 'MIG010_REPAIR_POLICY_V1' || POLICY.version !== '1.0.0') {
...
function applyRepairResolution(input) {
  ...
  for (const group of classified.duplicate_groups) {
    ...
    if (decision.decision === 'PRESERVE_ALL') {
      blockers.push('CANONICAL_IDENTITY_EXTENSION_REQUIRED');
      continue;
    }
    ...
    const selectedItem = classified.unique.find(() => false) || (() => {
      for (const raw of sourceRecords) {
        try {
          const normalized = normalizeSourceRecord(raw);
          if (normalized.source_quality !== 'VALID') continue;
          if (sourceFingerprint(normalized) !== group.fingerprint) continue;
          if (normalized.source_row !== decision.keep_source_row) continue;
          return { raw, normalized, fingerprint: group.fingerprint, index: sourceRecords.indexOf(raw) };
        } catch (_) { /* ignore */ }
      }
      return null;
    })();
  ...
  const candidate = sortedBlockers.length === 0
    ? validateCanonicalCollection(selected.map((item) => defaultSourceToCanonical(item.raw)))
    : Object.freeze([]);
