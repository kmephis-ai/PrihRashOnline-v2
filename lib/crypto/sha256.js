  ...
  for (let index = 0; index < text.length; index += 1) {
    ...
    if (first >= 0xd800 && first <= 0xdbff) {
      ...
      if (second >= 0xdc00 && second <= 0xdfff) {
        pushCodePoint(bytes, 0x10000 + ((first - 0xd800) << 10) + (second - 0xdc00));
        index += 1;
      } else {
        pushCodePoint(bytes, 0xfffd);
      }
...
function assertSha256Implementation() {
  const vectors = [
    ['', 'e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855'],
    ['abc', 'ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad'],
    ['Привет, мир', '2a2e76364df5ab8f0441d9c88bf7688f7f565f0f6b92a877cc94263e123021e3'],
    ['😀', 'f0443a342c5ef54783a111b51ba56c938e474c32324d90c3a60c9c8e3a37e2d9']
  ];
  for (const [input, expected] of vectors) {
    if (sha256Hex(input) !== expected) {
      const error = new Error('SHA256_IMPLEMENTATION_INVALID');
      error.code = 'SHA256_IMPLEMENTATION_INVALID';
      throw error;
    }
  }
  return true;
...
const IMPLEMENTATION_VALID = assertSha256Implementation();
