...
const NAMESPACE_RE = /^[A-Za-z0-9][A-Za-z0-9._:@/+|-]{0,191}$/;
...
function assertContract() {
  ...
  if (CONTRACT.key.projection_identity_required_when_projection_capability !== true ||
      !CONTRACT.key.components.includes('repository_adapter_namespace') ||
      !CONTRACT.key.components.includes('projection_identity')) {
    fail('REVISION_CACHE_KEY_NAMESPACE_POLICY_INVALID');
  }
  if (CONTRACT.freshness.revision_required_before_hit !== true ||
      ...
      CONTRACT.freshness.projection_change !== 'MISS') {
...
function normalizeNamespace(value, fallback, reason) {
  const text = value == null || String(value).trim() === '' ? fallback : String(value).trim();
  if (!NAMESPACE_RE.test(text)) fail(reason);
  return text;
}

function normalizeOptions(options = {}) {
  if (!options || typeof options !== 'object' || Array.isArray(options)) fail('REVISION_CACHE_OPTIONS_INVALID');
  const allowed = new Set(['ttl_ms', 'max_entries', 'now_ms', 'projection_identity']);
  ...
  const projectionIdentity = options.projection_identity == null
    ? null
    : normalizeNamespace(options.projection_identity, 'NONE', 'REVISION_CACHE_PROJECTION_IDENTITY_INVALID');
...
function repositoryAdapterNamespace(repository) {
  const adapterSchema = normalizeNamespace(
    repository.adapter_schema || repository.adapter,
    'GENERIC_REPOSITORY',
    'REVISION_CACHE_ADAPTER_NAMESPACE_INVALID'
  );
  const adapterVersion = normalizeNamespace(
    repository.adapter_version,
    'UNVERSIONED',
    'REVISION_CACHE_ADAPTER_VERSION_INVALID'
  );
  const mappingVersion = normalizeNamespace(
    repository.mapping_version,
    'NO_MAPPING',
    'REVISION_CACHE_MAPPING_VERSION_INVALID'
  );
  return `${adapterSchema}@${adapterVersion}|mapping:${mappingVersion}`;
}

function resolveProjectionIdentity(repository, options) {
  const projectionCapable = !!(repository.capabilities && repository.capabilities.projection === true);
  if (projectionCapable && !options.projection_identity) {
    fail('REVISION_CACHE_PROJECTION_IDENTITY_REQUIRED');
  }
  return options.projection_identity || 'NO_PROJECTION';
}
...
function normalizeKeyNamespace(input = {}) {
  if (!input || typeof input !== 'object' || Array.isArray(input)) fail('REVISION_CACHE_KEY_NAMESPACE_INVALID');
  for (const key of Object.keys(input)) {
    if (!['repository_adapter_namespace', 'projection_identity'].includes(key)) {
      fail('REVISION_CACHE_KEY_NAMESPACE_FIELD_UNKNOWN', key);
    }
  }
  return Object.freeze({
    repository_adapter_namespace: normalizeNamespace(
      input.repository_adapter_namespace,
      'GENERIC_REPOSITORY@UNVERSIONED|mapping:NO_MAPPING',
      'REVISION_CACHE_ADAPTER_NAMESPACE_INVALID'
    ),
    projection_identity: normalizeNamespace(
      input.projection_identity,
      'NO_PROJECTION',
      'REVISION_CACHE_PROJECTION_IDENTITY_INVALID'
    )
  });
}

function cacheKeyHash(repositorySchema, revision, operation, input, namespaceInput = {}) {
  const identityHash = operationIdentityHash(operation, input);
  const namespace = normalizeKeyNamespace(namespaceInput);
  return sha256(stableStringify({
    cache_schema: CONTRACT.schema,
    cache_version: CONTRACT.version,
    repository_schema: repositorySchema,
    repository_adapter_namespace: namespace.repository_adapter_namespace,
    projection_identity: namespace.projection_identity,
...
function createRevisionAwareReadCache(repositoryInput, optionsInput = {}) {
  ...
  const keyNamespace = Object.freeze({
    repository_adapter_namespace: repositoryAdapterNamespace(repository),
    projection_identity: resolveProjectionIdentity(repository, options)
  });
  ...
  function execute(operation, input, loader) {
    ...
    const key = cacheKeyHash(repository.schema, revision, operation, input, keyNamespace);
    ...
    if (cached) {
      ...
      if (ageMs >= 0 && ageMs < options.ttl_ms && cached.revision === revision &&
          ...
          cached.repository_adapter_namespace === keyNamespace.repository_adapter_namespace &&
          cached.projection_identity === keyNamespace.projection_identity) {
        touch(key, cached);
        setTelemetry({
          cache_status: 'HIT',
    ...
    entries.set(key, Object.freeze({
      cache_schema: CONTRACT.schema,
      cache_version: CONTRACT.version,
      repository_adapter_namespace: keyNamespace.repository_adapter_namespace,
      projection_identity: keyNamespace.projection_identity,
      revision,
  ...
  return Object.freeze({
    ...
    cache_namespace: Object.freeze({ ...keyNamespace }),
