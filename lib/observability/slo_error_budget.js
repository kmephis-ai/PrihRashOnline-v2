...
function assertTechnicalCode(value, code) {
  if (typeof value !== 'string' || !/^[A-Z][A-Z0-9_.:-]{0,63}$/.test(value)) fail(code);
  return value;
}

function assertContract() {
  if (CONTRACT.schema !== 'PRH_SLO_ERROR_BUDGET_V1' ||
      CONTRACT.version !== '1.0.0' || CONTRACT.roadmap_id !== 'OBS-010') {
    fail('SLO_CONTRACT_INVALID');
  }
  ...
  if (CONTRACT.privacy.financial_payload_allowed !== false ||
      ...
      CONTRACT.correctness_signal.financial_values_allowed !== false ||
      CONTRACT.correctness_signal.raw_payload_allowed !== false ||
  ...
  for (const sli of SLI_IDS) {
    ...
    if (!Number.isInteger(profile.min_samples) || profile.min_samples <= 0) {
      fail('SLO_MIN_SAMPLES_INVALID', sli);
    }
    if ((profile.kind === 'LATENCY_THRESHOLD_RATIO' || profile.kind === 'AGE_THRESHOLD_RATIO') &&
        !isSafeNonNegativeInteger(profile.threshold_ms)) {
      fail('SLO_THRESHOLD_INVALID', sli);
    }
  }
  return true;
}
...
function observationSpec(sli) {
  const profile = CONTRACT.sli_profiles[sli];
  if (!profile) fail('SLO_ID_UNKNOWN', String(sli));
  switch (profile.kind) {
    case 'BOOLEAN_OK_RATIO':
      return { allowed: ['sli', 'ts_ms', 'ok'], value: 'ok' };
    case 'CORRECTNESS_EVIDENCE_RATIO':
      return { allowed: ['sli', 'ts_ms', 'ok', 'source'], value: 'correctness' };
    case 'LATENCY_THRESHOLD_RATIO':
      return { allowed: ['sli', 'ts_ms', 'latency_ms'], value: 'latency_ms' };
    case 'AGE_THRESHOLD_RATIO':
      return { allowed: ['sli', 'ts_ms', 'age_ms'], value: 'age_ms' };
    case 'ERROR_FREE_RATIO':
      return { allowed: ['sli', 'ts_ms', 'error'], value: 'error' };
    default:
      fail('SLO_KIND_UNSUPPORTED', profile.kind);
  }
}
...
function normalizeObservation(input) {
  ...
  const spec = observationSpec(sli);
  const allowed = new Set(spec.allowed);
  for (const key of Object.keys(source)) {
    if (!allowed.has(key)) fail('SLO_OBSERVATION_FIELD_UNKNOWN', `${sli}:${key}`);
  }
  ...
  if (spec.value === 'ok') {
    ...
    output.ok = source.ok;
  } else if (spec.value === 'correctness') {
    if (typeof source.ok !== 'boolean') fail('SLO_OBSERVATION_VALUE_INVALID', sli);
    const evidenceSource = assertTechnicalCode(source.source, 'SLO_CORRECTNESS_SOURCE_INVALID');
    if (!CONTRACT.correctness_signal.allowed_sources.includes(evidenceSource)) {
      fail('SLO_CORRECTNESS_SOURCE_NOT_ALLOWED', evidenceSource);
    }
    output.ok = source.ok;
    output.source = evidenceSource;
  } else if (spec.value === 'error') {
    if (typeof source.error !== 'boolean') fail('SLO_OBSERVATION_VALUE_INVALID', sli);
    output.error = source.error;
  } else {
    if (!isSafeNonNegativeInteger(source[spec.value])) fail('SLO_OBSERVATION_VALUE_INVALID', sli);
    output[spec.value] = source[spec.value];
  }
...
function isGood(sli, observation) {
  ...
  switch (profile.kind) {
    ...
    case 'LATENCY_THRESHOLD_RATIO':
      return observation.latency_ms <= profile.threshold_ms;
    case 'AGE_THRESHOLD_RATIO':
      return observation.age_ms <= profile.threshold_ms;
    case 'ERROR_FREE_RATIO':
...
function maxState(left, right) {
  if (STATE_RANK[left] === undefined) fail('SLO_STATE_INVALID', left);
  if (STATE_RANK[right] === undefined) fail('SLO_STATE_INVALID', right);
...
function computeBudget(consumedBadPpm, allowedBadPpm) {
  if (allowedBadPpm === 0) {
    return consumedBadPpm === 0 ? 0 : BPS + 1;
  }
  return Math.ceil((consumedBadPpm * BPS) / allowedBadPpm);
}

function evaluateSli(input) {
  assertContract();
  const source = input && typeof input === 'object' && !Array.isArray(input) ? input : {};
  const allowedInput = new Set(['service', 'window', 'sli', 'telemetry_state', 'observations']);
  for (const key of Object.keys(source)) {
    if (!allowedInput.has(key)) fail('SLO_INPUT_FIELD_UNKNOWN', key);
  }

  const service = assertTechnicalCode(source.service, 'SLO_SERVICE_INVALID');
  ...
  const scoped = normalized.filter((observation) =>
  ...
  const goodCount = scoped.reduce((count, observation) => count + (isGood(sli, observation) ? 1 : 0), 0);
  const badCount = scoped.length - goodCount;
  const observedGoodPpm = Math.floor((goodCount * PPM) / scoped.length);
  ...
  const consumedBps = computeBudget(consumedBadPpm, allowedBadPpm);
...
function evaluateService(input) {
  assertContract();
  const source = input && typeof input === 'object' && !Array.isArray(input) ? input : {};
  const allowedInput = new Set(['service', 'window', 'telemetry_state', 'observations']);
  for (const key of Object.keys(source)) {
    if (!allowedInput.has(key)) fail('SLO_SERVICE_INPUT_FIELD_UNKNOWN', key);
  }
  const service = assertTechnicalCode(source.service, 'SLO_SERVICE_INVALID');
  const window = normalizeWindow(source.window);
  const telemetryState = normalizeTelemetryState(source.telemetry_state);
  ...
  const results = SLI_IDS.map((sli) => evaluateSli({
    ...
    sli,
    telemetry_state: telemetryState,
    observations: normalized.filter((observation) => observation.sli === sli)
  }));
...
function toAuditMetadata(result) {
  if (!result || typeof result !== 'object' || result.schema !== CONTRACT.schema ||
      result.version !== CONTRACT.version || !SLI_IDS.includes(result.sli)) {
    fail('SLO_AUDIT_RESULT_INVALID');
  }
  return Object.freeze({
    sliId: result.sli,
    sloStatus: result.status,
    sloObjectivePpm: result.objective_ppm,
    sloThresholdMs: result.threshold_ms === null ? undefined : result.threshold_ms,
    sloSampleCount: result.sample_count,
    sloBadSampleCount: result.bad_sample_count,
    sloRemainingBudgetPpm: result.remaining_budget_ppm,
    sloBudgetBurnBps: result.budget_consumed_bps,
    sloBudgetState: result.budget_state,
    sloReasonCode: result.reason_code
  });
}
