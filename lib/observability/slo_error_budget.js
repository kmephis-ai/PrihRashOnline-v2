...
function assertContract() {
  ...
  if (!CONTRACT.correctness_signal ||
      CONTRACT.correctness_signal.semantics !== 'PASS_FAIL_MACHINE_EVIDENCE_ONLY' ||
      CONTRACT.correctness_signal.financial_values_allowed !== false ||
      !Array.isArray(CONTRACT.correctness_signal.allowed_sources) ||
      CONTRACT.correctness_signal.allowed_sources.length === 0) {
    fail('SLO_CORRECTNESS_SIGNAL_INVALID');
  }
  for (const source of CONTRACT.correctness_signal.allowed_sources) {
    assertCode(source, 'SLO_CORRECTNESS_SOURCE_INVALID');
  }
  ...
  switch (profile.kind) {
    case 'BOOLEAN_OK_RATIO':
      return Object.freeze({
        fields: sli === 'CORRECTNESS' ? ['sli', 'ts_ms', 'ok', 'source'] : ['sli', 'ts_ms', 'ok'],
        value: 'ok'
      });
...
function normalizeObservation(input) {
  ...
  if (shape.value === 'ok') {
    ...
    output.ok = source.ok;
    if (sli === 'CORRECTNESS') {
      const evidenceSource = assertCode(source.source, 'SLO_CORRECTNESS_SOURCE_INVALID');
...
function evaluateSli(input) {
  ...
  const consumedBps = allowedBadPpm === 0
    ? (consumedBadPpm === 0 ? 0 : BPS + 1)
    : Math.ceil((consumedBadPpm * BPS) / allowedBadPpm);
...
function toAuditMetadata(result) {
  if (!result || typeof result !== 'object' || result.schema !== CONTRACT.schema || !SLI_IDS.includes(result.sli)) {
  ...
  return Object.freeze({
    ...
    sloBadSampleCount: result.bad_sample_count === null ? undefined : result.bad_sample_count,
    sloRemainingBudgetPpm: result.remaining_budget_ppm === null ? undefined : result.remaining_budget_ppm,
    sloBudgetConsumedBps: result.budget_consumed_bps === null ? undefined : result.budget_consumed_bps,
