...
function assertContract() {
  ...
  if(CONTRACT.session.integrity!=='HMAC_SHA256_RUNTIME_INJECTED_KEY'||CONTRACT.session.minimum_key_bytes<32||CONTRACT.session.constant_time_signature_compare!==true) fail('AUTH_SESSION_POLICY_INVALID');
...
function createSessionStore() {
  ...
  return Object.freeze({
    ...
    rotate(sessionId, nextVersion) { const value=sessions.get(sessionId); if(!value) fail('AUTH_SESSION_STORE_MISSING'); value.current_version=nextVersion; value.used_nonce_hashes=new Set(); sessions.set(sessionId,value); },
...
function startSession(principal, key, store, options={}) {
  ...
  const payload={schema:SESSION_SCHEMA,version:VERSION,session_id:sessionId,subject_id:principal.subject_id,household_id:principal.household_id,role:principal.role,issued_at:now,last_activity_at:now,absolute_expires_at:now+CONTRACT.session.absolute_lifetime_seconds,idle_timeout_seconds:CONTRACT.session.idle_timeout_seconds,session_version:1};
  store.put(sessionId,{current_version:1,revoked:false,subject_id:principal.subject_id,household_id:principal.household_id,role:principal.role});
...
function verifySession(token, key, store, options={}) {
  ...
  if(record.subject_id!==payload.subject_id||record.household_id!==payload.household_id||record.role!==payload.role) fail('AUTH_SESSION_BINDING_INVALID');
  const issuedAt=safeEpoch(payload.issued_at,'AUTH_SESSION_IAT_INVALID');
  const lastActivity=safeEpoch(payload.last_activity_at,'AUTH_SESSION_ACTIVITY_INVALID');
  const absoluteExpires=safeEpoch(payload.absolute_expires_at,'AUTH_SESSION_EXP_INVALID');
  const idleTimeout=safeEpoch(payload.idle_timeout_seconds,'AUTH_SESSION_IDLE_INVALID');
  if(absoluteExpires<=now) fail('AUTH_SESSION_ABSOLUTE_EXPIRED');
  if(lastActivity+idleTimeout<=now) fail('AUTH_SESSION_IDLE_EXPIRED');
  if(lastActivity<issuedAt||issuedAt>now+60) fail('AUTH_SESSION_TIME_INVALID');
  return Object.freeze({...payload,session_state:'ACTIVE'});
}
...
function rotateSession(token, key, store, options={}) {
  ...
  store.rotate(current.session_id,nextVersion);
  const next={...current}; delete next.session_state;
  next.session_version=nextVersion;
...
function verifyAndConsumeMutationNonce(nonceToken, session, capability, key, store, now) {
  ...
  const nonceHash=sha256(nonceToken);
...
function authorize(token, request, key, store, options={}) {
  ...
  try { session=verifySession(token,key,store,options); } catch(error) { return deny(String(error.code||'AUTH_SESSION_INVALID'),null,capability||null); }
...
function decisionTelemetry(decision, token, principal, count=1) {
  if(!decision||decision.schema!==DECISION_SCHEMA) fail('AUTH_DECISION_INVALID');
  const telemetry=Object.freeze({schema:TELEMETRY_SCHEMA,version:VERSION,decision:decision.decision,reason_code:decision.reason_code,role:decision.role,capability:decision.capability,session_state:decision.session_state,principal_hash:principal?sha256(`${principal.issuer}|${principal.subject_id}`):null,household_hash:principal?sha256(principal.household_id):null,session_hash:token?sha256(token):null,decision_count:Number(count)});
