...
function roundBigRatioHalfAway(numerator, denominator) {
  if (typeof numerator !== 'bigint' || typeof denominator !== 'bigint') fail('CALC_RATIO_BIGINT_INVALID');
  if (denominator === 0n) fail('CALC_RATIO_DENOMINATOR_ZERO');
  const negative = (numerator < 0n) !== (denominator < 0n);
  let top = numerator < 0n ? -numerator : numerator;
...
function roundRatioHalfAway(numerator, denominator) {
  ...
  return roundBigRatioHalfAway(BigInt(numerator), BigInt(denominator));
}
...
function topNOtherResult(source, spec) {
  ...
  const sorted = source.rows.map((row) => Object.freeze({
    ...
    const byValue = compareIntegers(left.value_minor, right.value_minor);
    return byValue === 0 ? left.key.localeCompare(right.key) : -byValue;
  });
...
function medianRounded(values) {
  const sorted = values.slice().sort(compareIntegers);
  ...
  source.primary_buckets.forEach((primaryBucket, index) => {
    ...
    dimensions.forEach((dimensionObject, key) => {
      ...
      if (spec.operator === 'DELTA_ABS') {
        ...
        valuePpm = roundBigRatioHalfAway(BigInt(delta) * BigInt(RATIO_SCALE), BigInt(reference));
      }
