...
function normalizeSort(value, measures, dimensions) {
  if (value == null) return Object.freeze([]);
  ...
  const normalized = value.map((item) => {
    ...
    if (kind === 'DIMENSION' && !dimensions.includes(key) && key !== 'time_bucket') fail('ANALYTICS_SORT_DIMENSION_NOT_SELECTED');
...
function normalizeAnalyticsQuery(input) {
  ...
  const dimensions = input.dimensions == null
    ? Object.freeze([])
    : normalizeStringArray(input.dimensions, DIMENSIONS, CONTRACT.limits.max_dimensions, 'ANALYTICS_DIMENSIONS_INVALID', true);
  ...
  const sort = normalizeSort(input.sort, measures, dimensions);
...
function zeroMeasures(measures) {
  return Object.freeze(Object.fromEntries(measures.map((measure) => [measure, 0])));
}

function evaluateMeasures(transactions, query) {
  if (transactions.length === 0) return zeroMeasures(query.measures);
...
function compareRows(left, right, query) {
  for (const sort of query.sort) {
    let comparison;
    if (sort.kind === 'MEASURE') comparison = left.measures[sort.key] - right.measures[sort.key];
    else comparison = compareScalar(left.dimensions[sort.key], right.dimensions[sort.key]);
