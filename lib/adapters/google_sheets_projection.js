...
function normalizeHeaderPositions(headers, requiredHeaders) {
  ...
  const relevant = new Set(Array.isArray(requiredHeaders) ? requiredHeaders : MAPPING.required_headers);
  const positions = new Map();
  normalized.forEach((header, index) => {
    if (!header || !relevant.has(header)) return;
    if (positions.has(header)) fail('GOOGLE_PROJECTION_SOURCE_HEADER_DUPLICATE', header);
...
function buildColumnSpans(headers, requiredHeaders) {
  ...
  const positions = normalizeHeaderPositions(headers, requested);
