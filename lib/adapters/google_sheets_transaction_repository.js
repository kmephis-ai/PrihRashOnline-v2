...
function matchesProjectedCandidate(candidate, query) {
  if (query.transaction_ids && !query.transaction_ids.includes(candidate.transaction_id)) return false;
  if (query.types && !query.types.includes(candidate.type)) return false;
  if (query.statuses && !query.statuses.includes(candidate.status)) return false;
  if (query.currency && candidate.currency !== query.currency) return false;
  if (query.account_id && candidate.account_id !== query.account_id && candidate.destination_account_id !== query.account_id) return false;
  if (query.category_id && candidate.category_id !== query.category_id) return false;
  if (query.member_id && candidate.member_id !== query.member_id) return false;
  if (query.project_id && candidate.project_id !== query.project_id) return false;
  if (query.tags_any && !query.tags_any.some((tag) => candidate.tags.includes(tag))) return false;
...
function compareProjectedCandidates(left, right) {
...
function createGoogleSheetsTransactionRepository(gateway, options = {}) {
  ...
  function queryProjected(queryInput) {
    ...
    snapshot.rows.forEach((row, offset) => {
      ...
      if (matchesProjectedCandidate(candidate, query)) candidates.push(candidate);
    });
    candidates.sort(compareProjectedCandidates);
...
module.exports = {
  ...
  parseTags,
  rowToCanonical,
  matchesProjectedCandidate,
  compareProjectedCandidates,
