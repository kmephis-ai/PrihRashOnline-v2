...
const LATEST_MONTH_SCHEMA = 'PRH_GOOGLE_LATEST_MONTH_SNAPSHOT_V1';
...
function validateNormalizedCanonicalCollection(transactions) {
  if (!Array.isArray(transactions)) fail('CANONICAL_COLLECTION_INVALID');
  const transactionIds = new Set();
  const sourceIds = new Set();
  for (const tx of transactions) {
    if (!tx || tx.schema !== 'PRH_CANONICAL_TRANSACTION_V1' || tx.schema_version !== 1) {
      fail('CANONICAL_TRANSACTION_SHAPE_INVALID');
    }
    if (transactionIds.has(tx.transaction_id)) fail('CANONICAL_TRANSACTION_ID_DUPLICATE');
...
function createGoogleSheetsTransactionRepository(gateway, options = {}) {
  ...
  function readCanonical(startRow, rowCount) {
    ...
    return validateNormalizedCanonicalCollection(canonicalFromSnapshot(snapshot));
  }

  function readLatestCalendarMonth() {
    const timelineHeaders = ['ID', 'Дата и время'];
    const timeline = readProjected(timelineHeaders);
    const index = normalizeHeaderIndex(timeline.headers, timelineHeaders);
    const candidates = [];
    const ids = new Set();
    let latestMs = null;

    timeline.rows.forEach((row, offset) => {
      const transactionId = normalizedLabel(row[index.ID]);
      if (!transactionId) return;
      if (ids.has(transactionId)) fail('GOOGLE_ADAPTER_TRANSACTION_ID_DUPLICATE');
      ids.add(transactionId);
      const occurredAt = toRfc3339(row[index['Дата и время']]);
      const occurredMs = Date.parse(occurredAt);
      if (!Number.isFinite(occurredMs)) fail('GOOGLE_ADAPTER_OCCURRED_AT_INVALID');
      if (latestMs == null || occurredMs > latestMs) latestMs = occurredMs;
      candidates.push({
    ...
    if (latestMs == null || candidates.length === 0) fail('GOOGLE_ADAPTER_LATEST_MONTH_UNAVAILABLE');

    const latest = new Date(latestMs);
    const start = new Date(Date.UTC(latest.getUTCFullYear(), latest.getUTCMonth(), 1)).toISOString().slice(0, 10);
    const end = new Date(Date.UTC(latest.getUTCFullYear(), latest.getUTCMonth() + 1, 1)).toISOString().slice(0, 10);
    const selected = candidates.filter((candidate) => {
      const day = candidate.occurred_at.slice(0, 10);
      return day >= start && day < end;
    });
    if (selected.length === 0) fail('GOOGLE_ADAPTER_LATEST_MONTH_EMPTY');
    ...
    validateNormalizedCanonicalCollection(items);

    return Object.freeze({
      schema: LATEST_MONTH_SCHEMA,
      period: Object.freeze({ start, end, partial: false }),
