...
function normalizeExportCustomizationDescriptors(input, configuration) {
  ...
  const normalized = input.map((descriptor) => {
    ...
    const applied = CUSTOM.applyCustomization(binding, descriptor.customization);
    if (applied.query_hash !== binding.query_hash || applied.query_modified !== false) fail('DASH086_CUSTOMIZATION_QUERY_IDENTITY_CHANGED', widgetId);
    return deepFreeze({
...
function validateEnvelopeObject(input) {
  ...
  if (input.portable_hash !== rawPortableHash || input.checksum !== `${CONTRACT.checksum.prefix}${rawPortableHash}`) {
    fail('DASH086_ENVELOPE_CHECKSUM_MISMATCH');
  }

  const payload = normalizePayload(input.payload);
  const configuration = SAVED.normalizeConfiguration(payload.configuration);
  const payloadByteCount = byteLength(stableStringify(payload));
  const counts = countPayload(configuration, payload.customizations, payloadByteCount);
  ...
  if (canonical.payload_hash !== input.payload_hash || canonical.portable_hash !== input.portable_hash || canonical.checksum !== input.checksum) {
    fail('DASH086_REEXPORT_IDENTITY_MISMATCH');
  }
...
function migrateLegacyObject(legacyInput) {
  ...
  const configurationInput = {
    schema: SAVED.CONFIG_SCHEMA,
    contract_version: SAVED.VERSION,
    dashboard_spec: legacyInput.dashboard_spec,
    bound_widgets: legacyInput.bound_widgets
  };
  ...
  const receiptBody = deepFreeze({
    ...
    target_portable_hash: envelope.portable_hash,
    status: 'PASS'
  });
...
function reexportImported(imported) {
  ...
  if (imported.schema !== IMPORT_RESULT_SCHEMA || imported.contract_version !== VERSION || imported.status !== 'VALID' ||
      imported.decision !== 'DRY_RUN_ONLY' || imported.persistence_performed !== false || imported.persistence_authority !== false) {
    fail('DASH086_IMPORT_RESULT_INVALID');
  }
...
function telemetry(action, source, decision = 'ACCEPTED', reason = 'OK') {
  ...
  const output = deepFreeze({
    ...
    payload_hash_prefix: envelope.payload_hash.slice(0, 12),
    byte_count: envelope.counts.byte_count,
    widget_count: envelope.counts.widget_count,
    binding_count: envelope.counts.binding_count,
    customization_count: envelope.counts.customization_count,
...
module.exports = Object.freeze({
  ...
  MANIFEST_SCHEMA,
  PAYLOAD_SCHEMA,
  CUSTOMIZATION_DESCRIPTOR_SCHEMA,
