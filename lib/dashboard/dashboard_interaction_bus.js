...
function rawFactoryPresentation(input) {
  const kind = String(input && input.kind || '').toUpperCase();
  const presentation = input && input.presentation;
  if ((kind === 'KPI' || kind === 'CARD') && presentation && presentation.mode === kind) {
    return {
...
function normalizeFactoryBinding(input) {
  ...
  const base = Object.fromEntries(baseKeys.map((key) => [key, input[key]]));
  base.presentation = rawFactoryPresentation(input);
  const normalized = FACTORY.normalizeBinding(base);
...
function nextGlobalFilterContext(explorationSession, event) {
  ...
  const normalized = VIZ.normalizeFilterContext({ schema: VIZ.FILTER_CONTEXT_SCHEMA, contract_version: VIZ.VERSION, filters });
  return deepFreeze({
    schema: normalized.schema,
    contract_version: normalized.contract_version,
    filters: normalized.filters.map((item) => ({
      kind: item.kind,
      field: item.field,
