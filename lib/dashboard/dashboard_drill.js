...
function hierarchy(registry, hierarchyId) {
  ...
  if (id === 'TIME') return deepFreeze({ id: 'TIME', dimension: 'time_bucket', levels: Object.freeze(TIME_LEVELS.slice()), nodes: null });
...
function hierarchyTransition(registry, hierarchyId, fromLevel, toLevel) {
  ...
  const fromIndex = h.levels.indexOf(String(fromLevel || ''));
  const toIndex = h.levels.indexOf(String(toLevel || ''));
...
function timeSelectionWindow(level, selection) {
  ...
  if (level === 'DAY' && /^(\d{4})-(\d{2})-(\d{2})$/.test(text)) {
    ...
    return { start: text, end: new Date(d.getTime() + 86400000).toISOString().slice(0, 10) };
  }
...
function canonicalRawFilterContext(input) {
  const normalized = VIZ.normalizeFilterContext(input);
  return Object.freeze({
    raw: deepFreeze(rawFilterContext(normalized)),
...
function ensureSourceCompatibility(bound, h, action) {
  ...
  } else if (!q.dimensions.includes(h.dimension)) fail('DASH083_SOURCE_DIMENSION_NOT_BOUND');
}
...
function normalizeState(input) {
  ...
  const normalized = makeState(stateBody(input));
...
function dispatch(sessionInput, boundInput, registryInput, actionInput) {
  ...
  const next = makeState({
    ...
    hierarchy_id: h.id,
    level: action.action === 'DOWN' ? action.to_level : action.from_level,
...
function sourceQueryFilterContext(query) {
  const filters = (query.filters || []).map((item) => ({ kind: 'DIMENSION', field: item.field, operator: 'INCLUDE', values: item.values.slice() }));
...
function buildDrillThroughRequest(sessionInput, boundInput) {
  ...
  const serializedContext = canonicalRawFilterContext(rawFilterContext(merged));
  ...
  const body = deepFreeze({
    ...
    filter_context: serializedContext.raw,
    tx_query: {
    ...
    context_hash: serializedContext.context_hash,
...
function telemetry(action, result) {
  ...
  const output = Object.freeze({
    ...
    widget_hash_prefix: sha256(session?.present?.source_widget_id || request?.source_widget_id || receipt?.source_widget_id || '').slice(0, 12),
