...
const householdViz = require('../visualization/household_visual_projection');
...
function homeChartSpec_(widgetId) {
  const widget = homeWidgetSpecs().find((item) => item.id === widgetId);
  if (!widget || !widget.chart_spec) fail('HOME_CHART_SPEC_REQUIRED');
  return widget.chart_spec;
}

function compileHouseholdCashFlowChart(periodResults) {
  const dataset = householdViz.cashFlowRenderDataset(periodResults);
  return householdViz.compileHouseholdChart(homeChartSpec_('home-cash-flow-trend'), dataset);
}

function compileHouseholdExpenseMixChart(expenseEntries, topN = householdViz.DEFAULT_EXPENSE_TOP_N) {
  const dataset = householdViz.expenseMixRenderDataset(expenseEntries, topN);
  return householdViz.compileHouseholdChart(homeChartSpec_('home-expense-mix'), dataset);
}
...
module.exports = Object.freeze({
  ...
  buildAlerts,
  homeWidgetSpecs,
  compileHouseholdCashFlowChart,
  compileHouseholdExpenseMixChart
});